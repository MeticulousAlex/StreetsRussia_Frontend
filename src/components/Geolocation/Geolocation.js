...
import geolocationIcon from '../../images/geolocation-icon.svg';
import cancelIsYourCity from '../../images/cancel_is_your_city.svg';

export default function Geolocation({city, setCity, isNotificationShown, setIsNotificationShown, setIsCitiesPopupOpen}){

    const [coords, setCoords] = React.useState({lat: 55.751244, lon:37.618423})
    ...
    return(
        <div className='geolocation'>
            <div className='geolocation__button' onClick={openCitiesPopup}>
                ...
                <button className='geolocation__place-name' type='button'>{city}</button>
            </div>
            <Notification isNotificationShown={isNotificationShown} setIsNotificationShown={setIsNotificationShown} city={city} setCity={setCity} setIsCitiesPopupOpen={setIsCitiesPopupOpen}/>
        </div>
    )
}
